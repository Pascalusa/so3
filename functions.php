 ...
 * @package So_Lunch
 */
require_once( TEMPLATEPATH . '/inc/backup-website.php');

//wp-content/themes/soluch2/so3/inc/backup-website.php
...
add_action( 'add_meta_boxes', 'bowl_add_meta_box' );

function bowl_html( $post) {
	wp_nonce_field( '_bowl_nonce', 'bowl_nonce' ); ?>

	<p>
		<label for="bowl_bowl"><?php _e( 'bowl', 'bowl' ); ?></label><br>
		<input type="text" name="bowl_bowl" id="bowl_bowl" value="<?php echo bowl_get_meta( 'bowl_bowl' ); ?>">
	</p><?php
}

function bowl_save( $post_id ) {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
	if ( ! isset( $_POST['bowl_nonce'] ) || ! wp_verify_nonce( $_POST['bowl_nonce'], '_bowl_nonce' ) ) return;
	if ( ! current_user_can( 'edit_post', $post_id ) ) return;

	if ( isset( $_POST['bowl_bowl'] ) )
		update_post_meta( $post_id, 'bowl_bowl', esc_attr( $_POST['bowl_bowl'] ) );
}
add_action( 'save_post', 'bowl_save' );

/*
	Usage: bowl_get_meta( 'bowl_bowl' )
*/
